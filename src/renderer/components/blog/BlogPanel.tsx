...
export function BlogPanel({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <h2 className="text-xl font-bold tracking-tight text-white">Blog Studio</h2>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  <h4 className="font-medium text-white truncate">{blog.title}</h4>
              ...
              <button
                ...
                className="mt-3 text-xs text-brand-400 transition-colors duration-200 hover:text-brand-300"
              >
                {expandedId === blog.id ? 'Collapse' : 'Expand'}
              </button>
              {expandedId === blog.id && (
                <div className="mt-3 whitespace-pre-wrap rounded-lg bg-gray-800/50 p-4 text-sm leading-relaxed text-gray-300 animate-fade-in">
                  {blog.body}
                </div>
              )}
            </div>
